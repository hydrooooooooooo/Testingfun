...
async function testImprovedExtraction() {
  try {
    ...
    const resultsPath = path.join(__dirname, '../../data/improved_extraction_results.json');
    ...
    if (previewItems.length > 0) {
      for (let i = 0; i < Math.min(3, previewItems.length); i++) {
        ...
        logger.info(`Item ${i + 1}:`, { 
          title: item.title,
          price: item.price,
          desc: item.desc?.substring(0, 50) + (item.desc?.length > 50 ? '...' : ''),
    ...
    const allItemsPath = path.join(__dirname, '../../data/all_items_results.json');
    fs.writeFileSync(allItemsPath, JSON.stringify(allItems, null, 2));
    
    logger.info(`All items saved to ${allItemsPath}`);
    
    // Update the session with improved preview items
    updateSessionWithImprovedItems(previewItems);
    
    return { success: true, previewCount: previewItems.length, allItemsCount: allItems.length };
  } catch (error) {
