...
export default function InstructionsCard() {
  const steps = [
    ...
    },
    {
      ...
      icon: <Download className="w-6 h-6 text-steel-600" />,
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {steps.map((step, index) => (
            <div key={step.id} className="relative">
              <div className="bg-white rounded-2xl p-8 shadow-lg border border-cream-200 h-full">
                {/* Icône */}
                ...
                {/* Numéro d'étape */}
                <div className="absolute -top-3 -right-3 w-8 h-8 bg-navy text-white rounded-full flex items-center justify-center text-sm font-semibold">
                  {step.id}
                </div>
              ...
              {index < steps.length - 1 && (
                <div className="hidden md:block absolute top-1/2 -right-4 transform -translate-y-1/2 z-10">
                  <div className="w-8 h-8 bg-white rounded-full flex items-center justify-center shadow-md border border-cream-300">
